...
public class WhenCloseTo : Spec
{
    ...
    public void GivenCloseTo_ThenDoesNotThrow(int days, int toleranceDays)
        =>The<TimeSpan>().Is().CloseTo(The<TimeSpan>().Add(TimeSpan.FromDays(days)), TimeSpan.FromDays(toleranceDays));

    [Fact]
    public void GivenFail_ThenGetException()
    {
        var a = Another<TimeSpan>();
        ...
        ex.HasInnerMessage($"Expected a to be close to {b} but found {a}");
    }
}