    ...
    where TService : class
{
    ...
    {
        var theValue = value();
        _spec.AddArrangement(() => _spec.GetMock<TService>().SetReturnsDefault(theValue));
        _spec.AddArrangement(() => _spec.GetMock<TService>().SetReturnsDefault(Task.FromResult(theValue)));
